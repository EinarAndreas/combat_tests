...
```
The "covariate"-column is made based on the "code"-column, but might not match  to what were actually used for all samples. The last two columns are from the GEO deposit and reveal that 3 of the samples do not have data and will be removed from the sampleannotation.

The naming convention seem to differ slightly between the Table S1 and the text in Towfic et al. This is our interpretation of covariate labels and its corresponding name in the text.
...
There seems to two linear relationship, one for negative FC's and another for positive FC's. They seems to have different intercepts but comparable slope.
