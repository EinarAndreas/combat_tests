...
```
A look at the most interesting groups, "DP"(GA) and "N"(generic) reveals a lack of balance.
```{r}
...
Several tables with results are presented in the Supporting Information in Towfic et al. We are aiming to reproduce two of these. Unfortunatly they were only presented in a pdf-format not easely parsed by a computer. We had to resort to a ad-hoc method of cut-and-paste from pdf into text files which were somewhat polluted by pdf-formatting code. For some tables about 10% of the rows will be lost, but the rest will suffice for our purpose.
...
Towfic et al. performed many different test on the data and it proved difficult and to labouros to reproduce all of their results without any source code. Insted we foucus on the key part of testing for differentially expressed genes between "GA" (DP) and "generic"(N) as described in Table S2 and Table S5. But before those tests we have to pre-process according to the description.
...
foldchange_DP_N = meanDP - meanN
 ...
 # make the axis equal.
lim =c( min(   c(min(table_s5$Fold_Change, na.rm=T), min(foldchange_DP_N[table_s5$Probe], na.rm=T))  ), 
        max( c(max(table_s5$Fold_Change, na.rm=T), max(foldchange_DP_N[table_s5$Probe], na.rm=T)) ))
...
The reproduced fold changes for the probes listed in Table S5 are clearly more extreme than the rest.


The next test is a test for differentially expressed genes using ANOVA.
